...
// displays most recent EONET event when page loads
function getEvent() {
  ...
  fetch(categorySearch)
    ...
    .then(function (data) {
      ...
      // if statement writes N/A if no description is found
...
// allows users to search for an event by category
function categorySearch() {
  // input from search dropdown is spliced into a link
  ...
  fetch(categorySearchUrl)
    ...
    .then(function (data) {
      ...
      // appends event names as buttons to modal, adds id and styling classes to each
      ...
      // event listeners for each event option, passes event url to categorySearchBtn()
...
// event listener for category search field
$("#categories-search").click(function () {
  categorySearch();
  console.log("test");
});

// allows users to search for events under a specific category
